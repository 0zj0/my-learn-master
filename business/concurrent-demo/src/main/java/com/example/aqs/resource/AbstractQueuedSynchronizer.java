...
public abstract class AbstractQueuedSynchronizer
    ...
    /**
     * setState方法通常用于当前正持有锁的线程对state变量进行修改，不存在竞争，
     * 是线程安全的，所以此处没必要用CAS保证原子性，修改的性能更重要。
     */
    ...
    /**
     * compareAndSetState通常用于在获取到锁之前，尝试加锁时，对state进行修改，
     * 这种场景下，由于当前线程不是锁持有者，所以对state的修改是线程不安全的，
     * 也就是说可能存在多个线程都尝试修改state,所以需要保证对state修改的原子性操作，即使用了unsafe类的本地CAS方法；
     */
    ...
    /**
     * 节点加入CLH同步队列:
     * CLH的tail尾节点为null,创建新的Node节点，CAS设置CLH队列的head头节和尾节点为创建的空节点，
     * 并将CLH的tail尾节点指向当前head头节点，然后循环次方法；
     * CLH的tail尾节点不为null,当前Node节点prev头部指向CLH尾节点，CLH的tail尾节点指向当前节点,
     * 前Node的next前驱节点的next指针指向当前节点，并返回当前节点
     */
    ...
    /**
     *当前CLH队列为空，pred 为null,走enq()；
     * 当前CLH队列不为空，当前Node节点prev头部指向CLH尾节点，
     * CLH的tail尾节点指向当前节点,当前Node的next前驱节点的next指针指向当前节点
     */
    ...
    /**
     * waitStatus ：标记当前节点的信号量状态 (1,0,-1,-2,-3)5种状态使用CAS更改状态，volatile保证线程可见性，高并发场景下，即被一个线程修改后，状态会立马让其他线程可见。
     * CANCELLED：1   在同步队列中等待的线程等待超时或者被中断，需要从同步队列中取消等待
     * SIGNAL ： -1   后继节点的线程处于等待状态，而当前的节点如果释放了同步状态或者被取消，将会通知后继节点，使后继节点的线程得以运行。
     * CONDITION：-2  节点在等待队列中，节点的线程等待在Condition上，当其他线程对Condition调用了signal()方法后，该节点会从等待队列中转移到同步队列中，加入到同步状态的获取中
    ...
    /**
     * 获取锁：
     * 1.存在其他线程锁未释放，当前锁中断，阻塞
     * 2.不存在需要释放的锁，当前线程若成功竞争到锁，加锁成功，执行下面逻辑
     * 3.不存在需要释放的锁，当前线程未成功竞争到锁，当前锁中断，阻塞
     */
    public final void acquire(int arg) {
        //tryAcquire 尝试获取锁，还未加锁（首次进入，锁重入），返回true; 已存在锁未释放，返回false
        //addWaiter： 将当前线程生产Node并加到CLH队列中
        if (!tryAcquire(arg) &&
